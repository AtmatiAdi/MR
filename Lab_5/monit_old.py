    ...
    while True:
        try:
            ...
            if exists(robot_file):
                ...
                robot_pos_plot = ax.plot(robot_pos[1], robot_pos[0], marker="^", markersize=10, color=(1,0,0))

            if exists(goal_file):
                goal_pos = pickle.load(open(goal_file, 'rb'))

                goal_pos_plot = ax.plot(goal_pos[1], goal_pos[0], marker="*", markersize=10, color=(1,0.5,0))        
